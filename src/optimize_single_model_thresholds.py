"""
Optimize classification thresholds for a single trained model.
...
def generate_predictions(model, data_loader, device):
    ...
    Returns:
        predictions: (num_samples, num_classes) probabilities
    """
    model.eval()
    all_predictions = []
...
def optimize_thresholds(model, val_loader, val_labels, device='mps'):
    ...
    """
    print("\n" + "="*70)
    print("THRESHOLD OPTIMIZATION - SINGLE MODEL")
    print("="*70 + "\n")
    ...
    output_file = 'results/single_model_optimal_thresholds.json'
...
def main():
    ...
    checkpoint = torch.load('models/baseline_model.pth', map_location=device, weights_only=False)
    model.load_state_dict(checkpoint['model_state_dict'])
    model = model.to(device)
    model.eval()
    
    print(f"✓ Model loaded from models/baseline_model.pth")
