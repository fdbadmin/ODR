"""
Threshold Tuning for ViT-Base Model
===================================
...
Expected improvement: +4-6% F1 score (based on previous models)
"""
...
def main():
    print("="*80)
    print("  ViT-Base Threshold Optimization")
    print("="*80)
    
    # Configuration
    data_dir = 'preprocessed_data_smart_exclusion'
    model_path = 'models_vit_base/best_vit_base.pth'
    output_path = 'models_vit_base/optimized_thresholds.json'
    ...
    # Load model
    print(f"\n🔨 Loading ViT-Base model from {model_path}...")
    model = create_model('vit_base', num_classes=num_classes)
    ...
    # Get predictions
    probs, labels = get_predictions(model, val_dataset, device=device, batch_size=16)
